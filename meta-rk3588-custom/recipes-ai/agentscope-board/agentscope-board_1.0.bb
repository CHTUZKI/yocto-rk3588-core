...
do_check_vendor() {
    if [ ! -f "${TOPDIR}/../deploy/vendor/agentscope-aarch64-site.tar.gz" ]; then
        bbfatal "agentscope-board: missing deploy/vendor/agentscope-aarch64-site.tar.gz; run scripts/fetch-agentscope-aarch64.sh"
    fi
}
...
do_install() {
    ...
    tar -xzf ${WORKDIR}/agentscope-aarch64-site.tar.gz \
        -C ${D}/opt/agentscope/site-packages --strip-components=1
}
