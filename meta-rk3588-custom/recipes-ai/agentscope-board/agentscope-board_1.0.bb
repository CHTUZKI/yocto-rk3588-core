...
python do_check_vendor() {
    ...
    if not os.path.isfile(path):
        bb.fatal("agentscope-board: missing %s\n"
                 "Run: scripts/fetch-agentscope-aarch64.sh" % path)
    if os.path.getsize(path) < 1000000:
        bb.fatal("agentscope-board: vendor tarball looks too small: %s" % path)
}
...
do_install() {
    ...
    install -m 0755 ${WORKDIR}/run.sh ${D}/opt/agentscope/run.sh
    ...
    # file:// *.tar.gz is unpacked by bitbake into agentscope-site/
    if [ -d ${WORKDIR}/agentscope-site ]; then
        install -d ${D}/opt/agentscope/site-packages
        ...
        find ${D}/opt/agentscope/site-packages -type f -name '*.a' -delete
    else
...
INSANE_SKIP:${PN} += "already-stripped file-rdeps ldflags libdir staticdev"
